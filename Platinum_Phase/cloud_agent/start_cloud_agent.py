"""
Platinum Phase - Cloud Agent Starter
Launches Cloud Agent API with Swagger docs and orchestrator
"""
...
logger = logging.getLogger("cloud_agent_starter")


def start_cloud_api():
    """Start the Cloud Agent FastAPI server with Swagger docs."""
...
def start_cloud_orchestrator():
    ...
    for dir_name in ["Inbox", "Needs_Action", "Pending_Approval", "In_Progress", "Done", "Logs"]:
...
def main():
    logger.info("Starting AI Employee Vault - Cloud Agent (Platinum Phase)...")
    logger.info(f"Environment: {'production' if not os.getenv('DEV_MODE', 'true') == 'true' else 'development'}")
    logger.info(f"Vault path: {os.getenv('VAULT_PATH', '/app/vault')}")
